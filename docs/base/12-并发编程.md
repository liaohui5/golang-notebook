...
以上代码会不会等待 `washClothes` 和 `watchTV` 执行完成, 因为它们都被异步执行了,
不会再阻塞主线程, 而主线程执行到main函数最后一个输出数据就会结束并退出程序
...
func main() {
	...
	for wait > 0 {
		// 如果 wait 大于0说明有异步任务还未执行完成
		// 所以这里会阻塞主线程, 直到所有异步任务执行完成
	}
...
## 使用 sync 标准库优雅的等待异步任务执行完

- 必须要全局遍历, 利于维护
- 使用标准库封装好的方法, 而不是手动操作变量, 更直观易读
...
func watchTV(wg *sync.WaitGroup) {
...
func main() {
	...
	// 主goroutine等待,直到wg计数归零, 这个功能与手动等待的 for 循环类似
	wg.Wait()
...
2. 使用`go`关键字启动 `washClothes` 协程, WaitGroup 计数加一, 函数执行完之前 WaitGroup 计数减一
3. 使用`go`关键字启动 `watchTV()` 协程, WaitGroup 计数加一, 函数执行完之前 WaitGroup 计数减一
4. 主协程调用`wg.Wait()`等待计数器归零, 说明协程都执行完了
5. 主协程继续执行后续代码

![code-execution-flow](https://notebook-imgbed.s3.bitiful.net/notebook-imgbed/2026_06_18_731a2c3316633a1527464b478b2652f7.jpg)
...
- 互斥锁: 解决在多个异步执行的任务操作同一个数据时产生的数据竞争问题
- 并发安全: 保证多个异步任务操作的 `原子性`
- 什么是原子性: `可以将一个操作的完整成功看作为原子性`, 比如数据库事物, `一次性向修改多个数据表中的数据, 必须都成功, 否则回滚`
...
数据竞争导致程序执行的结果不可控, 这就是所谓的 `非并发安全`

### 互斥锁解决数据竞争

与数据库行锁/表锁类似, 互斥锁会让一个操作必须操作完成, 其他的线程才能操作
...
`数据竞争现象的本质是因为多个线程共享了同一个内存`, 为了防止这种竞争, 就需要加锁(读写锁/互斥锁)
...
那么不使用锁, 应该如何解决并发的数据竞争问题呢? 答案是使用 `channel`

## 通道 channel

中文翻译: `通道` `管道` `信道`, 主要有两个作用:

- 用来让协程(或者叫: `异步任务` `gorutine`)与主线程通信
- 用来解决 `加锁/解锁` 的问题, 保证操作的原子性
...
![channel-buffer](https://notebook-imgbed.s3.bitiful.net/notebook-imgbed/2026_06_18_aefe1c8b841cca12eb03dea530060b5e.jpg)
...
它用于在不同 goroutine 之间传递数据, 并且可以用来控制 goroutine 的生命周期

### Context 介绍

Context译为上下文, 是Go提供的一种并发控制的解决方案, 相比于管道和WaitGroup,
它可以更好的控制子孙协程以及层级更深的协程, Context本身是一个接口, 只要实现了该接口都
可以称之为上下文例如著名Web框架Gin中的 gin.Context, context标准库也提供了几个实现，分别是：

- emptyCtx: `background` 方法和 `todo` 方法 的返回值就是 `emptyCtx`
- cancelCtx: `withCancel` 方法的返回值就是 `cancelCtx`
- timerCtx: `withDeadline` 方法 和 `withTimeout` 方法的返回值就是 `timerCtx`
- valueCtx: `withValue` 方法的返回值就是 `valueCtx`, 用来储存/取值
...
- 不要将 `Context` 存入结构体,而应该显式地将 `Context` 作为第一个参数传递给需要它的函数, 通常命名为 `ctx`
- `Context` 是并发安全的,可以被任意多个 Goroutine 同时使用
- `Done()` 方法返回一个 `chan`,当该 `chan` 被关闭时,表示此 `Context` 被取消或超时
- `Err()` 方法返回取消的原因（例如 `context.Canceled` 或 `context.DeadlineExceeded`) 它是一个 `errors.New("message")`
